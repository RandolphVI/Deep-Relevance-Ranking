...
tf.flags.DEFINE_string("drmm_type", "TextDRMM", "Type of DRMM")
        ...
        with sess.as_default():
            ...
            with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
                learning_rate = tf.train.exponential_decay(learning_rate=FLAGS.learning_rate,
                                                           global_step=drmm.global_step, decay_steps=FLAGS.decay_steps,
                                                           decay_rate=FLAGS.decay_rate, staircase=True)
                optimizer = tf.train.AdamOptimizer(learning_rate)
                grads, vars = zip(*optimizer.compute_gradients(drmm.loss))
                grads, _ = tf.clip_by_global_norm(grads, clip_norm=FLAGS.norm_ratio)
                train_op = optimizer.apply_gradients(zip(grads, vars), global_step=drmm.global_step, name="train_op")
            ...
            # Summaries for loss
            loss_summary = tf.summary.scalar("loss", drmm.loss)
            ...
            current_step = sess.run(drmm.global_step)
            ...
            def train_step(x_batch, y_batch, y_batch_tuple):
                ...
                feed_dict = {
                    drmm.input_x: x_batch,
                    drmm.input_y_first: y_batch_first,
                    drmm.input_y_second: y_batch_second,
                    drmm.input_y_third: y_batch_third,
                    drmm.input_y: y_batch,
                    drmm.dropout_keep_prob: FLAGS.dropout_keep_prob,
                    drmm.is_training: True
                }
                _, step, summaries, loss = sess.run(
                    [train_op, drmm.global_step, train_summary_op, drmm.loss], feed_dict)
            ...
            def validation_step(x_val, y_val, y_val_tuple, writer=None):
                ...
                for batch_validation in batches_validation:
                    ...
                    feed_dict = {
                        drmm.input_x: x_batch_val,
                        drmm.input_y_first: y_batch_val_first,
                        drmm.input_y_second: y_batch_val_second,
                        drmm.input_y_third: y_batch_val_third,
                        drmm.input_y: y_batch_val,
                        drmm.dropout_keep_prob: 1.0,
                        drmm.is_training: False
                    }
                    step, summaries, scores, cur_loss = sess.run(
                        [drmm.global_step, validation_summary_op, drmm.scores, drmm.loss], feed_dict)
            ...
            for batch_train in batches_train:
                ...
                current_step = tf.train.global_step(sess, drmm.global_step)
