...
from text_pacrr import TextPACRR
...
tf.flags.DEFINE_string("pacrr_type", "TextPACRR", "Type of PACRR")
...
tf.flags.DEFINE_integer("evaluate_every", 250, "Evaluate model on dev set after this many steps (default: 5000)")
...
tf.flags.DEFINE_integer("checkpoint_every", 250, "Save model after this many steps (default: 1000)")
...
def train_pacrr():
    ...
    # Build a graph and pacrr object
        ...
        with sess.as_default():
            ...
            with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
                learning_rate = tf.train.exponential_decay(learning_rate=FLAGS.learning_rate,
                                                           global_step=pacrr.global_step, decay_steps=FLAGS.decay_steps,
                                                           decay_rate=FLAGS.decay_rate, staircase=True)
                optimizer = tf.train.AdamOptimizer(learning_rate)
                grads, vars = zip(*optimizer.compute_gradients(pacrr.loss))
                grads, _ = tf.clip_by_global_norm(grads, clip_norm=FLAGS.norm_ratio)
                train_op = optimizer.apply_gradients(zip(grads, vars), global_step=pacrr.global_step, name="train_op")
            ...
            # Summaries for loss
            loss_summary = tf.summary.scalar("loss", pacrr.loss)
            ...
            current_step = sess.run(pacrr.global_step)
            ...
            def train_step(x_batch, y_batch, y_batch_tuple):
                ...
                feed_dict = {
                    pacrr.input_x: x_batch,
                    pacrr.input_y_first: y_batch_first,
                    pacrr.input_y_second: y_batch_second,
                    pacrr.input_y_third: y_batch_third,
                    pacrr.input_y: y_batch,
                    pacrr.dropout_keep_prob: FLAGS.dropout_keep_prob,
                    pacrr.is_training: True
                }
                _, step, summaries, loss = sess.run(
                    [train_op, pacrr.global_step, train_summary_op, pacrr.loss], feed_dict)
            ...
            def validation_step(x_val, y_val, y_val_tuple, writer=None):
                ...
                for batch_validation in batches_validation:
                    ...
                    feed_dict = {
                        pacrr.input_x: x_batch_val,
                        pacrr.input_y_first: y_batch_val_first,
                        pacrr.input_y_second: y_batch_val_second,
                        pacrr.input_y_third: y_batch_val_third,
                        pacrr.input_y: y_batch_val,
                        pacrr.dropout_keep_prob: 1.0,
                        pacrr.is_training: False
                    }
                    step, summaries, scores, cur_loss = sess.run(
                        [pacrr.global_step, validation_summary_op, pacrr.scores, pacrr.loss], feed_dict)
                    ...
                    for i in range(FLAGS.top_num):
                        batch_predicted_onehot_labels_tk = dh.get_onehot_label_topk(scores=scores, top_num=i+1)
            ...
            for batch_train in batches_train:
                ...
                current_step = tf.train.global_step(sess, pacrr.global_step)
